      ...
      <div className='w-full h-full flex items-center justify-between py-[2%] md:py-0 px-[5%] md:px-[2%] relative bg-[#f5f6f0] z-[9999]'>
        ...
        >
          <img 
            ...
            alt='ChatGPTLogo' 
          />
          <span className='hidden md:block text-lg 1200size:text-2xl 1400size:text-3xl violetFont'>ChatGPT</span>
        </button>
        ...
        <div className='block md:hidden w-[50%]' onClick={toggleMenu}>
          {isOpen ? (
            // X 아이콘 (메뉴가 열려있을 때)
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className={`size-8 hamburger-icon ${isOpen ? 'hamburger-icon-active' : ''}`}>
        ...
        {/* 모바일 네비게이션 */}
        <div className={`bg-[#f5f6f0] mt-[10%] w-full absolute top-0 left-0 px-[5%] block md:hidden transition-all duration-500 ${isOpen ? 'h-[100vh]' : 'h-0'} overflow-hidden`}>
        ...
        <div className="flex-shrink-0 relative flex w-auto md:w-[15%] justify-end h-full">
          ...
          >
            ...
            <div className="btn-primary-text p-2 ml-2 1200size:p-4 1200size:ml-4 violetFont text-sm md:text-lg">Three GPT</div>
          </a>
        </div>
