    ...
    <div className="flex flex-col  text-lg">
      {qaData.map((qa, index) => (
        <div key={index} className="">
          ...
          <div className={`p-8 ${index === 0 ? 'border-t-0' : ''}`}>
            A. {qa.answer}
          </div>
        </div>
      ))}
...
<div className="w-[50%]">
  <ul className="text-right">
    {rightText.map((item, index) => (
      <li
        key={index}
        className={`flex justify-end   ${index === 0 ? 'border-r border-b border-t border-[#efefe5]  mb-4' : 'border border-[#353539] pt-2 pr-4 pb-[0.4375rem]'} pl-[42%]`}
      >
        {/* 화살표 */}
        <img
        ...
        {index === 0 ? (
          <div className="flex items-center ">
            <p className="px-2">{item.subtext}</p> {/* 서브텍스트 */}
